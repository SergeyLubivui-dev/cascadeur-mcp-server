...
def main() -> None:
    ...
    for name, src, dst in packages:
        ...
        # tools_pro's "Open Panel" launches the external web panel; record where
        # the repo + a Python interpreter live so the in-app command can spawn it.
        if name == "tools_pro":
            venv_py = os.path.join(_ROOT, ".venv", "Scripts", "python.exe")
            env_py = os.path.join(dst, "_env.py")
            with open(env_py, "w", encoding="utf-8") as f:
                f.write("# generated by install_bridge.py\n")
                f.write("REPO_ROOT = %r\n" % _ROOT)
                f.write("VENV_PY = %r\n" % (venv_py if os.path.isfile(venv_py)
                                            else ""))
                f.write("PANEL_PORT = 8765\n")
